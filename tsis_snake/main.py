...
from color_palette import *   # цвета
from config import *          # настройки (ширина, высота и т.д.)
from game import (            # импорт классов игры
    Snake, Food, PowerUp, Obstacle,
    draw_grid, draw_hud,
    generate_obstacles, load_settings, save_settings
)
import db             # работа с базой данных

pygame.init()         # инициализация pygame
screen = pygame.display.set_mode((WIDTH, HEIGHT))  # создаём окно
pygame.display.set_caption("Snake TSIS4")          # название окна
...
# ── Подключение к БД (если нет Postgres — просто не работает, но игра не падает)
DB_OK = db.init_db()

# ── ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ ─────────────────────────────

def draw_button(text, rect, hover=False):
    # рисуем кнопку (меняет цвет при наведении)
...
def text_input_screen(prompt):
    """Экран ввода текста (например username)"""
    text = ""
    while True:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_RETURN and text.strip():
                    return text.strip()   # возвращаем введённый текст
                elif event.key == pygame.K_BACKSPACE:
                    text = text[:-1]      # удаление символа
                elif len(text) < 20 and event.unicode.isprintable():
                    text += event.unicode  # добавление символа
...
def main_menu():
    ...
    while True:
        mx, my = pygame.mouse.get_pos()  # позиция мыши
        screen.fill(colorBLACK)
...
def play_game(username):
    ...
    POWERUP_SPAWN_INTERVAL = 10000  # каждые 10 секунд
    ...
    while running:
        ...
        # повышение уровня → добавляем препятствия
        ...
        clock.tick(snake.get_speed_fps())  # скорость игры
